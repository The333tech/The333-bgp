...
class ReleaseMetadataTests(unittest.TestCase):
    ...
    def test_product_version_is_synchronized_across_release_files(self) -> None:
        ...
        badge_block = badge_rows[0]
        self.assertEqual(badge_block.count("<img "), 12)
        self.assertEqual(badge_block.count("<a "), 12)
        self.assertEqual(badge_block.count("<kbd>"), 12)
        self.assertEqual(badge_block.count("<br>"), 2)
        self.assertEqual(badge_block.count("&nbsp;"), 9)
        self.assertNotIn("<ruby>", badge_block)
    ...
    def test_github_actions_are_pinned_and_release_is_attested(self) -> None:
        ...
        self.assertIn("python3 scripts/extract-release.py dist/source-tree.tar.gz dist/release-tree", release)
        self.assertIn("path: dist/release-tree", release)
        ...
        self.assertIn("linux/amd64,linux/arm64", release)
    ...
    def test_install_and_update_have_realistic_disk_preflight(self) -> None:
        ...
        self.assertIn("MIN_PROJECT_FREE_MB=512", installer)
        self.assertIn("MIN_FRESH_DOCKER_READY_MB=4096", installer)
        self.assertIn("MIN_FRESH_DOCKER_INSTALL_MB=5120", installer)
        self.assertIn("MIN_REPAIR_FREE_MB=3072", installer)
        self.assertIn('check_disk_policy "${initial_disk_mode}"', installer)
        self.assertIn('check_disk_policy "fresh-docker-ready"', installer)
        self.assertGreaterEqual(installer.count('check_disk_policy "repair"'), 2)
        self.assertIn("Docker Root Dir", installer)
        self.assertIn("Dry-run: Docker installation would be offered", installer)
        self.assertIn("DEFAULT_MIN_UPDATE_FREE_BYTES=1073741824", controller)
        self.assertIn("DEFAULT_RECOMMENDED_UPDATE_FREE_BYTES=2147483648", controller)
        self.assertIn("DEFAULT_MIN_CORE_UPDATE_FREE_BYTES=2147483648", controller)
        self.assertIn("DEFAULT_RECOMMENDED_CORE_UPDATE_FREE_BYTES=4294967296", controller)
        self.assertIn("DEFAULT_MIN_PROJECT_UPDATE_FREE_BYTES=536870912", controller)
        self.assertIn("check_update_disk_space", controller)
        self.assertIn("Docker Root Dir", controller)
        ...
        update_project = controller[controller.index("update_project()") :]
        self.assertGreaterEqual(update_project.count("check_update_disk_space"), 2)
        self.assertLess(
