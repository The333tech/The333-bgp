...
В экспериментальном режиме готовых GHCR-образов локальная сборка не выполняется, поэтому пороги ниже:
...
Режим использует только подписанные multi-platform images с неизменяемыми `sha256`-ссылками. Текущий способ сборки из исходников остаётся стандартным и резервным. Архитектура, проверка и безопасное переключение описаны в [Предсобранных runtime-образах](PREBUILT_IMAGES.md).
