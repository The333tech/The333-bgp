...
Сам release archive `v0.82.4b` занимает около 350 КБ, а распакованный код и встроенные конфигурации — около 3 МБ. На рабочем стенде Docker images, build cache, данные и backups вместе занимают примерно 1,5–2 ГБ после установки.
...
В опциональном режиме готовых GHCR-образов локальная сборка не выполняется, поэтому пороги ниже:
...
Режим использует только подписанные multi-platform images с неизменяемыми `sha256`-ссылками. Способ сборки из исходников остаётся стандартным и резервным. После появления release manifest с images тот же интерактивный установщик запускает prebuilt-режим одной строкой:
