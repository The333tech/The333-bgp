...
Это быстрый beta-режим: готовые `amd64/arm64` images загружаются из GHCR по immutable SHA-256 digest. Скрипт читает ответы напрямую из терминала, поэтому команда остаётся интерактивной даже при запуске через pipe.

Резервная локальная сборка из исходников запускается без переменной режима:
...
- свободное место с учётом предстоящей операции и состояния Docker;
...
Release archive занимает несколько сотен килобайт, а распакованный код и встроенные конфигурации — около 3 МБ. На рабочем стенде Docker images, данные и backups вместе занимают примерно 1–2 ГБ; source-сборка дополнительно создаёт build cache.

Дисковый preflight использует разные пороги:

| Операция | Жёсткий минимум | Рекомендуется |
|---|---:|---:|
| Новая установка с готовым Docker | 4 ГБ | 6 ГБ |
| Новая установка вместе с Docker | 5 ГБ | 8 ГБ |
| Update с установленным routing-core | 1 ГБ | 2 ГБ |
| Update с полной сборкой routing-core | 2 ГБ | 4 ГБ |
| Repair с повторной сборкой | 3 ГБ | 5 ГБ |

Запас нужен на время локальной сборки GoBGP/backend/portal, одновременного хранения builder layers, старых и новых Docker images, pre-update backup и автоматического rollback. Перед Docker build проверка выполняется повторно.

В режиме готовых GHCR-образов локальная сборка не выполняется, поэтому пороги ниже:

| Операция в режиме `prebuilt` | Жёсткий минимум | Рекомендуется |
|---|---:|---:|
| Новая установка с готовым Docker | 2 ГБ | 3 ГБ |
| Новая установка вместе с Docker | 3 ГБ | 4 ГБ |
| Update | 1 ГБ | 2 ГБ |
| Repair | 1,5 ГБ | 3 ГБ |

Режим использует только проверенные multi-platform images с неизменяемыми `sha256`-ссылками. Сборка из исходников остаётся резервным и аудируемым способом. Тот же интерактивный установщик запускает prebuilt-режим одной строкой:
...
Архитектура, проверка и безопасное переключение описаны в [Предсобранных runtime-образах](PREBUILT_IMAGES.md).

Установщик определяет `Docker Root Dir`. Если Docker и `/opt/the333-bgp` находятся на разных файловых системах, свободное место проверяется отдельно: не менее 512 МБ для файлов проекта и полный порог операции для Docker storage. Глобальная очистка `docker system prune` автоматически не запускается, потому что она могла бы удалить cache и неиспользуемые images других проектов.
...
> **Важно:** full-container AmneziaWG — отдельный сценарий. Не заменяй его стандартным WireGuard RouterOS. Пошаговая настройка AWG и проверки описаны в [инструкции MikroTik + AWG](MIKROTIK_AWG.md); чистая установка на другом роутере пока не подтверждена.
...
Обновления доступны в портале на странице **Обновления**. После запуска здесь же отображаются этапы и итог. Не открывай второе обновление при кратком разрыве связи: портал продолжит проверять операцию автоматически.
...
Пока обновление выполняется, другие изменения в портале и фоновые задания приостановлены. После проверки новой версии страница перезагрузится сама. Если после перезапуска backend потребуется вход, авторизуйся в этом же окне: серверное обновление от этого не отменяется.

Если портал показывает **«Требуется проверка сервера»**, не запускай обновление повторно и не редактируй файлы установки. Сначала проверь VM и резервную копию:
...
Если сервисы исправны, версия соответствует текущей или предыдущей версии из окна обновления, а updater больше не выполняет операцию, подтверди восстановление состояния:

```bash
sudo env THE333_PROJECT_DIR=/opt/the333-bgp python3 /opt/the333-bgp/scripts/update-operation.py recover --confirm
```

Команда откажется снять блокировку при работающем updater, неизвестной версии или непрошедшей проверке готовности. Если она отказала, восстанови сервис из проверенного backup или обратись к специалисту; не удаляй файл состояния вручную.

Обычное обновление портала/backend не пересоздаёт `the333-gobgp-core`, поэтому BGP-сессия и уже опубликованные маршруты продолжают работать. **Переход с GoBGP 4.7.0 на 4.9.0 (в том числе на v0.90.1b) меняет routing-core:** BGP-сессия кратко прервётся, а MikroTik получит маршруты заново после проверки готовности. Перед запуском запланируй окно обслуживания и убедись, что свободно не менее 2 ГБ на диске с Docker; рекомендуется 4 ГБ. Штатный updater повторно проверит эту смену после загрузки релиза и откажется менять рабочие контейнеры, если места недостаточно. При обновлении с v0.90b версия Core не меняется.
