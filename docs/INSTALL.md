...
Если установка была сделана до появления host-side updater, на VM может ещё работать старый контейнер `the333-host-updater` с Docker socket. В v0.78+ это считается legacy-режимом; установщик v0.82b переносит обновление в systemd-сервис.
