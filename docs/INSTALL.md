...
Сам release archive `v0.82.2b` занимает около 350 КБ, а распакованный код и встроенные конфигурации — около 3 МБ. На рабочем стенде Docker images, build cache, данные и backups вместе занимают примерно 1,5–2 ГБ после установки.

Проверка требует именно **8 ГБ свободного места**, а не VM-диск размером 8 ГБ. Дополнительный запас нужен на время локальной сборки GoBGP/backend/portal, одновременного хранения старых и новых Docker images, pre-update backup и автоматического rollback. Рекомендация 12+ ГБ также учитывает обновления ОС, рост данных и повторные обновления без немедленной очистки Docker cache.
