...
make_backup() {
  ...
  tar \
    ...
    -czf "${archive}" \
    -C "${PROJECT_DIR}" \
    .env VERSION CHANGELOG.md docker-compose.yml docker-compose.portal.yml Dockerfile entrypoint.sh \
    requirements.txt app portal config data docs scripts install.sh LICENSE SECURITY.md \
...
runtime_is_ready() {
  ...
  local host health
  host="$(runtime_host)"
  health="$(curl -fsS "http://${host}:8088/health" 2>/dev/null)" || return 1
  printf '%s' "${health}" \
...
copy_release_files() {
  ...
  for item in app portal docs Dockerfile entrypoint.sh requirements.txt docker-compose.yml docker-compose.portal.yml VERSION CHANGELOG.md LICENSE SECURITY.md install.sh scripts update-manifest.json update-manifest.example.json .env.example .dockerignore .gitignore; do
    if [[ -e "${src}/${item}" ]]; then
