...
if grep -RIn --exclude-dir=.git --exclude-dir=.github --exclude='release-check.sh' -E 'the333ctl|OWNER/REPO|TODO_RELEASE|CHANGE_ME_REAL_SECRET' .; then
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  prepare_compose_env
  docker compose -f docker-compose.yml -f docker-compose.portal.yml config >/dev/null
else
