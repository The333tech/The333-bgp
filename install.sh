...
copy_project_files() {
  ...
  for item in app portal config docs Dockerfile entrypoint.sh requirements.txt docker-compose.yml docker-compose.portal.yml VERSION CHANGELOG.md LICENSE SECURITY.md scripts update-manifest.json update-manifest.example.json .env.example .gitignore install.sh; do
    if [[ -e "${src}/${item}" ]]; then
...
write_env() {
  ...
  local bind_ip local_as router_id nexthop peer_ip peer_as community web_user web_password web_password_hash update_url host_updater_token
  ...
  update_url="${PRODUCT_UPDATE_MANIFEST_URL:-https://raw.githubusercontent.com/The333tech/The333-bgp/main/update-manifest.json}"
