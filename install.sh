...
check_resources() {
  ...
  if [[ -n "${disk_kb}" && "${disk_kb}" =~ ^[0-9]+$ && "${disk_kb}" -lt "${MIN_FREE_DISK_KB}" ]]; then
    fail "not enough free disk space near ${PROJECT_DIR}: $((disk_kb / 1024)) MB available. Need at least 8 GB free; recommended ${RECOMMENDED_FREE_DISK_GB}+ GB."
  fi
...
copy_project_files() {
  ...
  for item in app portal docs Dockerfile entrypoint.sh requirements.txt docker-compose.yml docker-compose.portal.yml VERSION CHANGELOG.md LICENSE SECURITY.md scripts update-manifest.json update-manifest.example.json .env.example .dockerignore .gitignore install.sh; do
    if [[ -e "${src}/${item}" ]]; then
  ...
  if [[ "${preserve_config}" == "true" ]]; then
    ...
    if [[ -d "${src}/config" ]]; then
      ...
      for cfg in "${src}"/config/*; do
        ...
        [[ -e "${target}" ]] || cp -a "${cfg}" "${target}"
      done
    fi
    log "Пользовательские файлы config сохранены; добавлены только отсутствующие файлы поставки."
  else
...
write_env() {
  ...
  local bind_ip local_as router_id nexthop peer_ip peer_as community web_user web_password web_password_hash update_url host_updater_token generated_password
...
HOST_UPDATER_URL=http://the333-host-updater:8091
...
wait_for_services() {
  ...
  log "Ожидание готовности backend, GoBGP и портала (до 3 минут)..."
  while (( attempts > 0 )); do
    if curl -fsS "http://${bind_ip}:8088/health" 2>/dev/null \
        | python3 -c 'import json,sys; data=json.load(sys.stdin); raise SystemExit(0 if data.get("ok") and data.get("gobgp_ready") else 1)' \
      && curl -fsS -o /dev/null "http://${bind_ip}:8090/" 2>/dev/null; then
  ...
  fail "services did not become ready within 3 minutes; inspect container logs"
}
