...
HOST_UPDATER_URL = os.getenv("HOST_UPDATER_URL", "http://the333-host-updater:8091").strip().rstrip("/")
...
def auth_client_key(request: Request) -> str:
    ...
    return (forwarded or real_ip or client_host or "unknown")[:128]
...
def require_auth(request: Request, credentials: HTTPBasicCredentials = Depends(security)) -> str:
    ...
    pass_ok = verify_portal_password(credentials.password)
...
async def load_update_manifest() -> dict[str, Any]:
    ...
    try:
        ...
        log_exception("update manifest load failed", e)
        fallback["ok"] = False
        fallback["error"] = public_error("Не удалось загрузить список обновлений.")
        return fallback
    ...
    state.update(
        {
            ...
            "files": files,
            "notes": [
                "0.1 keeps existing JSON shapes unchanged.",
                "Future versions must add explicit migrations here before changing user data formats.",
            ],
        }
    )
...
def run_product_update_command(channel: str, version: str | None = None) -> dict[str, Any]:
    ...
    if PRODUCT_UPDATE_MODE == "host-updater":
        if not HOST_UPDATER_URL:
            raise RuntimeError("HOST_UPDATER_URL is empty")
        if not HOST_UPDATER_TOKEN or "CHANGE_ME" in HOST_UPDATER_TOKEN:
            raise RuntimeError("HOST_UPDATER_TOKEN is not configured")

        started = time.time()
        with httpx.Client(timeout=PRODUCT_UPDATE_TIMEOUT_SECONDS, follow_redirects=True) as client:
            response = client.post(
                f"{HOST_UPDATER_URL}/api/update",
        ...
        try:
            ...
            data = {"ok": False, "detail": response.text}
...
async def api_diagnostics_bundle(_: str = Depends(require_auth)) -> StreamingResponse:
    ...
    with zipfile.ZipFile(bundle, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        add_json(
            ...
            {
                "catalog": safe_file_info(SERVICE_CATALOG_FILE),
